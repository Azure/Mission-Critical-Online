...
![Global load balancer configuration](/docs/media/AlwaysOn-GlobalRouting.gif)
...
- Azure Front Door provides both a fully managed certificate service as well as a bring-your-own-certificate method.
...
- Azure Front Door provides WAF capabilities at a global traffic level, while Application Gateway WAF must be provided within each regional deployment stamp.
...
> If there are high-scale requirements associated with outbound traffic, consideration should be given to a dedicated Azure Firewall resource for a mission-critical AlwaysOn application, to mitigate risks associated with using a centrally shared resource, such as noisy neighbour scenarios.
...
- Availability Zones are treated as logical entities within the context of a single subscription, so different subscriptions might have a different zonal mapping for the same region.
  - For example, zone 1 in Subscription A could correspond to the same physical data centre as zone 2 in subscription B.
...
- Prioritise the use of _Calico Network Policies_ because it provides a richer feature set with broader community adoption and support.

---

|Previous Page|Next Page|
|:--|:--|
 [Deployment and Testing](./Deploy-Testing.md) |[Security](./Security.md)
 ---
