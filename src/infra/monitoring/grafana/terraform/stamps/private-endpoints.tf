...
resource "azurerm_private_dns_zone_virtual_network_link" "pgdb" {
  for_each              = local.stamps
  name                  = "${local.prefix}-${substr(each.value, 0, 5)}-pg-pl"
  resource_group_name   = azurerm_resource_group.rg[each.key].name
...
resource "azurerm_private_endpoint" "pgdb_primary" {
  for_each            = local.stamps
  name                = "${local.prefix}-${substr(each.value, 0, 5)}-pg-pe"
  location            = azurerm_resource_group.rg[each.key].location
  resource_group_name = azurerm_resource_group.rg[each.key].name
  subnet_id           = azurerm_subnet.snet_datastores[each.key].id

  private_dns_zone_group {
    name                 = "${local.prefix}-${substr(each.value, 0, 5)}-privatepgdb"
    private_dns_zone_ids = [azurerm_private_dns_zone.pgdb[each.key].id]
  }

  private_service_connection {
    name                           = "${local.prefix}-${substr(each.value, 0, 5)}-pg-psconn"
...
resource "azurerm_private_endpoint" "pgdb_replicas" {
  ...
  private_service_connection {
    ...
    private_connection_resource_id = azurerm_postgresql_server.pgreplica[each.key].id
    subresource_names              = ["postgresqlServer"]
    is_manual_connection           = false
  }
  tags = local.default_tags
