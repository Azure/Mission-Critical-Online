...
| **Risk**                   | **Impact/Mitigation/Comment**                | **Outage**                 |
| -------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------- |
| **Database/collection is renamed**     | Can happen due to mismatch in configuration when deploying – Terraform would overwrite the whole database, which could result in data loss (this can be prevented by using [database/collection  level locks](https://docs.microsoft.com/en-us/azure/cosmos-db/scripts/cli/sql/lock)). <br />**Application will not be able to access any data**. App configuration needs to be updated and pods restarted. | Yes                     |
