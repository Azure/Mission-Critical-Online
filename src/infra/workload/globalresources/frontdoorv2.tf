...
data "azurerm_monitor_diagnostic_categories" "frontdoorv2" {
  resource_id = azurerm_cdn_frontdoor_profile.main.id
}

resource "azurerm_monitor_diagnostic_setting" "frontdoorv2" {
  ...
  dynamic "log" {
    iterator = entry
    for_each = data.azurerm_monitor_diagnostic_categories.frontdoorv2.log_category_types
  ...
  dynamic "metric" {
    iterator = entry
    for_each = data.azurerm_monitor_diagnostic_categories.frontdoorv2.metrics
